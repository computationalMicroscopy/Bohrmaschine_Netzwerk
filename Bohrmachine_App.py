...
def compute_sensor_diagnostics(current_vals, settings, integrity, kuehlung, m):
    ...
    mapping = {
        "Mechanische Torsions-Überlast": {
            ...
            "act": f"MINDERN SIE DEN VORSCHUB: Reduzieren Sie f sofort um 30% auf {f*0.7:.2f} mm/U."
        },
        "Thermische Gefüge-Erweichung": {
            "diag": "THERMAL OVERLOAD",
            "exp": f"Die Schnittkantentemperatur ({T:.0f}°C) hat die Anlasstemperatur überschritten. Akute Gefügeerweichung!",
            "act": f"REDUZIEREN SIE REIBUNG: Schnittgeschwindigkeit vc um 25% auf {int(vc*0.75)} m/min drosseln."
        },
        "Regeneratives Rattern (Resonanz)": {
            "diag": "RESONANT CHATTER",
            "exp": f"Selbsterregte Schwingungen ({V:.1f} mm/s) stören den Spanbildungsprozess massiv.",
            "act": f"DREHZAHL-SHIFT ERFORDERLICH: Ändern Sie vc um +15% auf {int(vc*1.15)} m/min, um Resonanz zu brechen."
        },
        "Aufbauschneidenbildung (Adhäsion)": {
            "diag": "ADHESION DETECTED",
            "exp": "Die Schnittgeschwindigkeit ist zu gering. Werkstoffteilchen verschweißen kalt auf der Schneide.",
            "act": f"ERHÖHEN SIE vc: Fahren Sie vc um 20% hoch ({int(vc*1.2)} m/min) für eine thermodynamische Stabilisierung."
        },
        "Axiale Schaft-Knickung (Vorschub zu hoch)": {
            ...
            "act": f"VORSCHUB HALBIEREN: Reduzieren Sie f umgehend auf {f*0.5:.2f} mm/U gegen Kernbruch."
        },
        "Kühlungs-Abriss (Thermoschock-Gefahr)": {
            "diag": "TRIBOLOGY FAILURE",
            "exp": "Schlagartiger Temperaturanstieg mangels KSS-Medium. Direktes Zuschalten erzeugt Thermoschock-Risse.",
            "act": "ZYKLUS UNTERBRECHEN: Vorschub stoppen, Werkzeug rotierend aus der Bohrung fahren und an Luft kühlen."
        },
        "Spanstau / Spanflächen-Verstopfung": {
            "diag": "CHIP CLOGGING",
            "exp": "Drehmoment und Vibrationen steigen simultan. Späne blockieren die Spannuten.",
            "act": "PICK-FEEDING AKTIVIEREN: Fahren Sie einen Entspanungszyklus mit verkürzter Bohrtiefe pro Hub."
        },
        "Mikro-Bröckelung der Schneidkante": {
            "diag": "MICRO-CHIPPING",
            "exp": "Hochfrequente Vibrationsspitzen erzeugten feine Ausbrüche an der Schneidecke.",
            "act": "WERKZEUGPRÜFUNG: Vorschub f leicht reduzieren und Schneidkanten beim nächsten Stopp optisch prüfen."
        },
        "Schnittdaten-Unterforderung (Kaltverfestigung)": {
            "diag": "LOW ENGAGEMENT",
            "exp": "Der Vorschub ist zu gering, die Schneide schabt mehr als sie schneidet. Werkstoff verfestigt sich.",
            "act": f"SPANSTÄRKE ERHÖHEN: Steigern Sie den Vorschub f auf mindestens {max(0.12, f*1.5):.2f} mm/U."
        },
        "Abrasiver Freiflächenverschleiß (Normal)": {
            "diag": "ABRASIVE WEAR",
            "exp": f"Fortschreitender Verschleiß durch Carbide im Material. Restintegrität liegt bei {integrity:.1f}%.",
            "act": "STANDZEIT-MONITORING: Der Prozess läuft stabil. Werkzeugwechsel beim nächsten Wartungsfenster planen."
        },
        "Spindellager-Überlastung (Vibration resonant)": {
            "diag": "SPINDLE VIBRATION",
            "exp": "Starke Vibrationen trotz niedriger Drehmomentlast weisen auf eine Unwucht der Werkzeugspindel hin.",
            "act": "WUCHT PRÜFEN: Drehzahl absenken. Werkzeugaufnahme auf Achsversatz oder Verschmutzung untersuchen."
        },
        "Extrem-Vorschub-Stauchung": {
            "diag": "HIGH FEED HEAVE",
            "exp": "Der gewählte Vorschub zwingt die Querschneide zu extremer plastischer Deformation des Materials.",
            "act": "VORSCHUB ANPASSEN: Reduzieren Sie f auf maximal 0.30 mm/U, um axiale Kernkompression zu mindern."
        },
        "Kratzender Schnitt (Zentrierfehler)": {
            "diag": "CENTERING ERROR",
            "exp": "Hohe Axialkraft bei minimalem Drehmoment deutet darauf hin, dass die Zentrierspitze reibt.",
            "act": "ANBOHRUNG PRÜFEN: Vorbohrung kontrollieren. Spitzenwinkel des Anbohrers muss größer sein."
        },
        "KSS-Verdampfung (Kavitation)": {
            "diag": "COOLANT BOILING",
            "exp": "Temperatur liegt über 100°C trotz aktivem KSS. Das Kühlmedium verdampft direkt an der Wirkstelle.",
            "act": "DRUCK ERHÖHEN: Erhöhen Sie den KSS-Druck über interne Kühlung (IKZ), um das Medium flüssig zu halten."
        },
        "Erhöhte Reibung durch Verschleißmarkenbreite": {
            "diag": "FRICTION ELEVATION",
            "exp": "Das Drehmoment kriecht nach oben, bedingt durch vergrößerte Reibflächen verbrauchter Schneiden.",
...
# Referenzbasis: d=12mm entspricht 44px Breite. Skalierungsfaktor berechnen:
scale_factor = d / 12.0
drill_width_px = int(44 * scale_factor)
drill_tip_height_px = int(21 * scale_factor)

# Dynamische Keyframes für Erschütterungen (Vibration) & Spanflug-Offsets generieren
...
with col_animation:
    ...
    synchronized_vfx = ""
    if s['active']:
        # Partikelemission passt sich der skalierten Bohrerbreite an (Offsets dynamisch per CSS-Variablen)
        synchronized_vfx += f"""
        ...
        if kuehlung:
            ...
            """
        if t_val > 220:
            synchronized_vfx += '<div style="position: absolute; left: 50%; bottom: 35px; width: 18px; height: 18px; background: rgba(240,240,240,0.15); filter: blur(7px); border-radius: 50%; animation: @keyframes smoke_rise 0.3s infinite linear; transform: translate(-50%, 0);"></div>'
        if t_val > 420:
            synchronized_vfx += f"""
    ...
    with col_graph:
        if s['history']:
            ...
            fig = make_subplots(rows=3, cols=1, shared_xaxes=True, vertical_spacing=0.08,
                                subplot_titles=("Strukturelle Werkzeugintegrität (%)", "Kinetische Lastprofile (Nm & N)", "Prozessdynamik (mm/s & °C)"))
