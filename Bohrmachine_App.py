...
import pandas as pd
import numpy as np
from pgmpy.models import BayesianNetwork # Abwärtskompatibler Import
...
@st.cache_resource
def get_engine():
    # Nutzen von BayesianNetwork für breitere Kompatibilität
    ...
    for age in range(3):
        for load in range(2):
            for therm in range(2):
                for cool in range(2):
                    # Dynamische Score-Gewichtung (Therm & Cool dominieren für schnellere Reaktion)
...
if st.session_state.twin['active'] and not st.session_state.twin['broken']:
    ...
    s['logs'].insert(0, f"[{zeit}] ZYK {s['cycle']} | RISIKO: {risk:.1%} | KI-LOGIK: [A:{age_cat} L:{load_cat} T:{therm_cat} C:{cool_cat}]")
