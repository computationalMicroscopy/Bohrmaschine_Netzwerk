...
st.set_page_config(layout="wide", page_title="TwinPro V5.8 | Professional AI Lab", page_icon="⚙️")
...
st.html(f"""
    ...
    /* FIX: HÖHE EXAKT AUF 500PX GEZWUNGEN DAMIT GRAPH UND FEED BÜNDIG SIND */
    .xai-container {{ height: 500px; overflow-y: auto; padding-right: 5px; background: #0d1117; border-radius: 8px; border: 1px solid #30363d; padding: 10px; }}
    .xai-card {{ background: #161b22; border-left: 6px solid #58a6ff; padding: 18px; border-radius: 8px; margin-bottom: 12px; border-top: 1px solid rgba(255,255,255,0.02); }}
    ...
    .action-text {{ color: #ff7b72; font-weight: bold; font-size: 1.15rem; margin-top: 8px; padding-top: 8px; }}
...
def compute_sensor_diagnostics(current_vals, settings, integrity, kuehlung, m):
    ...
    mapping = {
        "Mechanische Torsions-Überlast": {"diag": "CRITICAL TORQUE", "exp": f"Drehmoment ({M:.1f} Nm) übersteigt Scherspannungsgrenze.", "act": f"MINDERN SIE DEN VORSCHUB auf {f*0.7:.2f} mm/U."},
        "Thermische Gefüge-Erweichung": {"diag": "THERMAL OVERLOAD", "exp": f"Schnittkantentemperatur ({T:.0f}°C) über Anlasstemperatur.", "act": f"Schnittgeschwindigkeit vc auf {int(vc*0.75)} m/min drosseln."},
        "Regeneratives Rattern (Resonanz)": {"diag": "RESONANT CHATTER", "exp": f"Selbsterregte Schwingungen ({V:.1f} mm/s) detektiert.", "act": f"Ändern Sie vc um +15% auf {int(vc*1.15)} m/min."},
        "Aufbauschneidenbildung (Adhäsion)": {"diag": "ADHESION DETECTED", "exp": "Schnittgeschwindigkeit zu gering. Kaltverschweißung droht.", "act": f"Fahren Sie vc auf ({int(vc*1.2)} m/min) hoch."},
        "Axiale Schaft-Knickung": {"diag": "AXIAL BUCKLED", "exp": f"Axiale Vorschubkraft ({F:.0f} N) nähert sich Knicklast.", "act": f"Reduzieren Sie f umgehend auf {f*0.5:.2f} mm/U."},
...
# --- 6. TABS & TELEMETRIE-OSZILLOSKOP (LAYOUT FIXED) ---
t1, t2 = st.tabs(["📈 Echtzeit-Telemetrie & Oszilloskop", "🔬 Labor-Sandbox: Szenario-Analyse"])

with t1:
    # PERFEKTE 1:1 AUFTEILUNG
    col_graph, col_log = st.columns([1, 1])
    
    with col_graph:
        st.markdown('<span class="val-title">📊 Sensor-Oszilloskop</span>', unsafe_allow_html=True)
        if s['history']:
            ...
            # Feste Höhe von 500px für perfekte Bündigkeit mit Feed
    ...
    with col_log:
        ...
        if s['logs']:
            # Der xai-container ist via CSS strikt auf height: 500px eingestellt
